...
def prepare_df_vle(df_csv, molec_dict, csv_name=None, drop_one=False):
    ...
    if "Hvap" in df_csv.columns:
        ...
        df_csv["Hvap kJ/kg"] = (
            df_csv["Hvap"]
            * 1000.0
            / df_csv["molecule"].apply(lambda molec: molec_dict[molec].molecular_weight)
        )
    ...
    df_csv.dropna(
        subset=["sim_liq_density", "sim_vap_density"], how="any", inplace=True
    )
    ...
    if drop_one:
        df_csv = df_csv.groupby(["molecule", "temperature"]).filter(
            lambda x: len(x) > 1
        )
    ...
    for group, values in df.groupby(["molecule"]):
        ...
        if all(x == temps[0] for x in temps):
            ...
            try:
                ...
                slope2, intercept2, r_value2, p_value2, std_err2 = linregress(
                    temps,
                    abs((liq_density - vap_density)) ** (1 / 0.32),
                )
        ...
        if group[0] not in ["R134", "R152"] and len(values) > 0:
            ...
            def calculate_objs(expt_values, sim_values, property_name, molecule_name):
                try:
                    ...
                    print(
                        f"Error in calculating {property_name} for {molecule_name}: {e}. Setting MSE, MAE, and MAPD to NaN"
                    )
            ...
            for prop in ["liq_density", "vap_density", "Pvap", "Hvap"]:
                mse, mapd, mae = calculate_objs(
                    values["expt_" + prop], values["sim_" + prop], prop, group[0]
                )
            ...
            for prop in ["Tc", "rhoc"]:
                mse, mapd, mae = calculate_objs(
                    np.array([values["expt_" + prop].values[0]]),
                    np.array([values["sim_" + prop].values[0]]),
                    prop,
                    group[0],
                )
                new_quantities["mse_" + prop] = mse
...
def plot_vle_envelopes(molec_dict, df_ff_dict, save_name=None):
    ...
    for i in range(len(df_ff_list)):
        ...
        if df_ff is not None and show_df:
            ...
            for x_prop in x_props:
                ...
                min_rho, max_rho = get_min_max(
                    min_rho, max_rho, means[x_prop].values, stds[x_prop].values
                )
            ...
            min_temp, max_temp = get_min_max(
                min_temp, max_temp, means["sim_Tc"].values, stds["sim_Tc"].values
            )
            ax2.errorbar(
                means["sim_rhoc"].values[0],
                means["sim_Tc"].values[0],
                xerr=1.96 * stds["sim_rhoc"].values[0],
                color=df_colors[i],
                markersize=10,
                linestyle="None",
                marker=df_marker,
    ...
    if molec not in ["R152", "R134"]:
        ax2.scatter(
            mol_data.expt_liq_density.values(),
            mol_data.expt_liq_density.keys(),
            color="black",
            marker="x",
            linewidths=2,
            s=100,
        ...
        ax2.scatter(
            mol_data.expt_vap_density.values(),
            mol_data.expt_vap_density.keys(),
            color="black",
            marker="x",
            linewidths=2,
            s=100,
    ...
    ax2.tick_params(
        "both", direction="in", which="both", length=4, labelsize=26, pad=10
    )
    ...
    ax2.set_xlabel(
        r"$\mathregular{\rho}$ (kg/m$\mathregular{^3}$)", fontsize=32, labelpad=15
    )
...
def plot_pvap_hvap(molec_dict, df_ff_dict, save_name=None):
    ...
    for i in range(len(df_ff_list)):
        ...
        if (
            df_ff is not None and show_df
        ):  # and ("Potoff" in df_label or "GAFF" in df_label)
            ...
            min_temp, max_temp = get_min_max(
                min_temp, max_temp, means["temperature"].values
            )
            ...
            if len(log_Pvap_finite) > 0:
                std_log_pvap = (stds["sim_Pvap"].values / means["sim_Pvap"].values)[
                    finite_indices
                ]
                ...
                min_pvap, max_pvap = get_min_max(
                    min_pvap, max_pvap, log_Pvap_finite, std_log_pvap
                )
                axs[0].errorbar(
                    1 / temps_finite,
                    log_Pvap_finite,
            ...
            if not np.all(np.isnan(means["sim_Hvap"].values)):
                ...
                min_hvap, max_hvap = get_min_max(
                    min_hvap, max_hvap, Hvap_finite, std_hvap
                )
    ...
    if molec not in ["R152", "R134"]:
        axs[0].scatter(
            1 / np.array(list(mol_data.expt_Pvap.keys())),
    ...
    if molec not in ["R152", "R134", "R143"]:
        axs[1].scatter(
            mol_data.expt_Hvap.keys(),
            mol_data.expt_Hvap.values(),
            color="black",
            marker="x",
            label="Experiment",
            s=100,
    ...
    axs[0].tick_params(
        "both", direction="in", which="both", length=2, labelsize=16, pad=10
    )
    ...
    axs[1].set_ylabel(
        r"$\mathregular{\Delta H_{vap}}$ (kJ/kg)", fontsize=16, labelpad=8
    )
    ...
    fig.subplots_adjust(
        bottom=0.15, top=0.85, left=0.15, right=0.85, wspace=0.55, hspace=0.5
    )
...
def plot_err_each_prop(molec_names, err_path_dict, obj="mapd", save_name=None):
    ...
    if obj == "mae":
        names = [
            "Liquid Density " + r"$(kg/m^3)$",
            "Vapor Density " + r"$(kg/m^3)$",
            "Vapor Pressure " + r"$(bar)$",
...
def plot_err_avg_props(molec_names, err_path_dict, obj="mapd", save_name=None):
    ...
    def compute_average_mapd(df, mapd_columns, obj):
        ...
        average_mapd = (
            df[mapd_columns].mean().sort_values().reset_index().iloc[:, [0, -1]]
        )
    ...
    for i in range(len(df_labels)):
        ...
        y_err = [
            merged_df[label][obj] - merged_df[label]["Min"],
            merged_df[label]["Max"] - merged_df[label][obj],
        ]
    ...
    ax_right.set_xlim(
        -0.4, len(merged_df.index) - len_train - 0.6
    )  # Adjust for test set size
...
def get_param_and_mapd(
    str_mol, at_list=["gaff", 1, 2], mapd_params=["rho_l", "rho_v", "P_vap", "H_vap"]
):
    molec_data = []
    upp_bnds = {
        "rho_l": 5,
        ...
        "P_vap": 5,
        "H_vap": 5,
    }  # Upper bounds for MAPD, init at 5
    for at_num in at_list:
        ...
        if isinstance(at_num, int):
            ...
            data_set = pd.read_csv(
                os.path.join(all_molec_dir, "unique_best_set.csv"),
                header=0,
                index_col=False,
            )
            # param_best = analyzer.values_pref_to_real()
            bnds = analyzer.at_class.at_bounds_nm_kjmol.T @ param_matrix
            param_best = values_real_to_scaled(
                analyzer.values_pref_to_real(
                    data_set.iloc[0].values @ param_matrix
                ).reshape(1, -1),
                bnds.T,
            )
            ...
            data_class = analyzer.all_molec_data[str_mol]
            molec_gaff = np.array(list(data_class.gaff_param_set.values()))
            bnds = analyzer.at_class.at_bounds_nm_kjmol.T @ param_matrix
            param_best = values_real_to_scaled(
                analyzer.values_pref_to_real(molec_gaff).reshape(1, -1), bnds.T
            )
        ...
        key_map = {
            "rho_l": "mapd_liq_density",
            "rho_v": "mapd_vap_density",
            "P_vap": "mapd_Pvap",
            "H_vap": "mapd_Hvap",
        }
        mapd = {}
    ...
    col_names = param_names + [
        "MAPD " + mapd_params[i] for i in range(len(mapd_params))
    ]
    ...
    for key in upp_bnds.keys():
        ...
        if "MAPD " + key in df.columns:
            ...
            df["MAPD " + key] = values_real_to_scaled(
                df["MAPD " + key].values, mapd_bnd
            )
...
def plot_param_comp(
    ...
    df_colors = [colors[at] for at in at_list]
    all_markers = {
        "gaff": "s",
        1: "o",
        2: "p",
    ...
    df, data_class, bnds, result_bounds = get_param_and_mapd(
        str_mol, at_list, mapd_params
    )
    ...
    for name in data_class.param_names:
        latex_name = lambda s: (
            rf"$\{s.split('_',1)[0]}_{{{s.split('_',1)[1]}}}$"
            if "_" in s
            else rf"${s}$"
        )
    ...
    mapd_names = ["MAPD\n" + make_mapd_name(name) for name in mapd_params]
    mapd_names = [name.replace("H_{vap}", "\Delta H_{vap}") for name in mapd_names]

    col_names = param_names + mapd_names
...
def get_mapd_train_test_avg(str_mol, at_list=["gaff", 1, 2], mode="train"):
    ...
    for at_num in at_list:
        ...
        for key, value in key_map.items():
            ...
            # err_val = errs.loc[errs["molecule"] == str_mol, value].values[0]
            err_val = np.nanmean(
                errs.loc[errs["molecule"].isin(all_mols), value].values
            )
            upp_bnd = 5 * np.ceil(err_val / 5)
            if upp_bnds[key] < upp_bnd:
                upp_bnds[key] = upp_bnd
...
def plot_mapd_comp(
    ...
    # Create (N-1) subplots along x axis
    n_axis = len(mapd_params)
    fig, all_axes = plt.subplots(2, n_axis - 1, sharey=False, figsize=(n_axis * 2, 12))
    for mode in ["train", "test"]:
        axes = all_axes[0].flatten() if mode == "train" else all_axes[1].flatten()
        df, data_class, bnds, result_bounds = get_mapd_train_test_avg(
            str_mol, at_list, mode=mode
        )
        seaborn.set_palette("bright", n_colors=len(df))
        data = df.to_numpy()
        # result_bounds = np.array([[0, 35], [0, 35], [0, 35], [0, 35]])  # MAPD bounds for rho_l, rho_v, H_vap, P_vap
        ...
        for dim, ax in enumerate(axes):
            ax.xaxis.set_major_locator(ticker.FixedLocator([dim]))
            set_ticks_for_axis(ax, bounds[dim], nticks=6)
            fontsize = 24 if dim < 10 else 20
            if mode == "test":
                ax.set_xticklabels([col_names[dim]], fontsize=fontsize)
            else:
                ax.set_xticklabels([])
            ax.set_ylim(-0.05, 1.05)
            # Add white background behind labels
